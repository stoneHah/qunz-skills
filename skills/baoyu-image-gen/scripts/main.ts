...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "replicate" && v !== "apimart") {
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        currentKey === "provider_limits" &&
        indent >= 4 &&
        (key === "google" || key === "openai" || key === "dashscope" || key === "replicate" || key === "apimart")
      ) {
        config.batch ??= {};
        config.batch.provider_limits ??= {};
...
function getConfiguredProviderRateLimits(
  ...
  for (const provider of ["replicate", "google", "openai", "dashscope", "apimart"] as Provider[]) {
